...
main(){
	...
	priority_queue<dj,vector<dj>,greater<dj>>q;q.push({0,0,0,0});
	while(q.size()){
		auto d=q.top();q.pop();
		if(d.node==n-1){cout<<(powmod(2,d.tr)-1+d.mv+mod)%mod<<endl;return 0;}
		if(!(dp[d.node][d.gt][min(19,d.tr)]>d))continue;
		dp[d.node][d.gt][min(19,d.tr)]=d;
		for(int i=0;i<K;++i)
			for(auto v:g[d.node][i])
				if(dp[v][i][min(19,d.tr+(d.gt!=i))]>(dj){d.tr+(d.gt!=i),d.mv+1,v,i})
					q.push({d.tr+(d.gt!=i),d.mv+1,v,i});
	}
}